...
export default function RateDetailsPage() {
  ...
  useEffect(() => {
    ...
    const record = rates.find((item) => item.id === decodeURIComponent(params?.id || ""));
    setRate(record || null);
  ...
  const save = (event) => {
    ...
    if (isDuplicateActive(rates, nextRecord, rate.id)) {
      setError("An active rate already exists for this customer, route and rate type.");
      return;
    }

    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(rates.map((item) => item.id === rate.id ? nextRecord : item)));
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold" style={{ color: NAVY }}>{rate.id}</h1>
