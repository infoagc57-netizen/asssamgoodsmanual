...
import { syncLedger } from "../accounts/page";
...
export default function CustomersPage() {
  ...
  const [outstandingByCustomerId, setOutstandingByCustomerId] = useState({});
  ...
  useEffect(() => {
    try {
      ...
      setOutstandingByCustomerId(ledger.reduce((totals, entry) => {
        if (!entry.customerId) return totals;
        totals[entry.customerId] = (totals[entry.customerId] || 0) + Number(entry.balance || 0);
        return totals;
      }, {}));
    } catch {
      setCustomers([]);
      setOutstandingByCustomerId({});
    }
  }, []);
